...
Original file is located at
    https://colab.research.google.com/drive/1XPyL9mahgaUxIuKhWSSG17QCV5Q-jw6Y
"""
...
true = Y_val[0].reshape((64,59))
pred = val_outputs[0].reshape((64,59))
...
total_true = np.zeros((64, 59))
total_pred = np.zeros((64, 59))

# Loop through the weeks
for week in range(0, 7):
    # Extract data for the current week
    inp_week = X_val[week][:, :, 0]
    true_week = Y_val[week].reshape((64, 59))
    pred_week = val_outputs[week].reshape((64, 59))
    print(pred_week)
...
# Save the map to an HTML file
m.save(f'Week_predicted.html')
m

"""Calculated montly data"""

# Initialize variables to store cumulative values
total_inp_month = np.zeros_like(X_val[0][:, :, 0])
total_true_month = np.zeros((64, 59))
total_pred_month = np.zeros((64, 59))

# Loop through the weeks
for month in range(0, 30):
    # Extract data for the current week
    inp_month = X_val[month][:, :, 0]
    true_month = Y_val[month].reshape((64, 59))
    pred_month = val_outputs[month].reshape((64, 59))
    print(pred_month)
    # Accumulate values element-wise
...
# Save the map to an HTML file
m.save(f'Monthly_predicted.html')
m
...
for i in range(65):
    ax = axs[i]
    prediction = val_outputs[i].reshape((64, 59))
    print(prediction)
...
val_outputs_flat1 = np.array(val_outputs)
# Calculate colorbar ticks based on the actual minimum and maximum values in the dataset
min_value = np.min(val_outputs_flat1)
max_value = np.max(val_outputs_flat1)
...
cbar_ax = fig.add_axes([0.3, 0.001, 0.35, 0.015])  # Adjust these values as needed
